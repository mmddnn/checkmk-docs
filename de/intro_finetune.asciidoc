...
weil das in {CMK} so einfach geht. Als sie dann kurz danach die Benachrichtigungen für
...
*Hinweis:* Fehlalarme können natürlich nur bei eingeschalteten Benachrichtigungen entstehen. Worum es im folgenden also geht, ist es die Vorstufe der Benachrichtigungen abzustellen und die kritischen Zustände {DOWN}, {WARN} oder {CRIT} für unkritische Probleme zu vermeiden.
...
// TK2SK: Regelabarbeitung der Reihe nach: Im CMK 1 Kurs hab ich gelernt, dass dies bei den Benachrichtigungsregeln nicht zutrifft, da die alle abgearbeitet und ausgewertet werden. Ist das 1 Anmerkung Wert - oder verwirrt es eher, da im Kap. Benachrichtigungen nicht um Benachrichtigungenregeln geht?
...
entweder das xref:intro_gui#search_setup[Suchfeld im Setup-Menü] nutzen oder über [.guihint]#Setup > General > Rule search# die Seite zur Regelsuche öffnen. Wir werden den letztgenannten Weg im folgenden Kapitel gehen, in dem wir die Erstellung einer Regel vorstellen.

Bei der großen Anzahl der verfügbaren Regelsätze ist es mit oder ohne Suche nicht immer einfach, den richtigen zu finden. Es gibt aber noch einen anderen Weg, mit dem Sie auf die passenden Regeln für einen bereits existierenden Service zugreifen können. Klicken Sie in einer Tabellenansicht mit dem Service auf das icon:icon_menu[alt="Symbol zum Öffnen des Aktionsmenüs."] Menü und wählen Sie den Eintrag [.guihint]#Parameters for this service#:
...
image::service_rule_icon.png[alt="Listeneintrag eines Services mit geöffnetem Aktionsmenü."]

Sie erhalten eine Seite, von der aus Sie Zugriff auf alle Regelsätze dieses Services haben:

image::parameters_of_this_service.png[alt="Liste aller Regelsätze für einen Service."]
...
image::ruleset_search_tablespace.png[alt="Ergebnis der Suche nach 'tablespace' in Regeln."]
...
image::ruleset_oracle_tablespaces.png[alt="Dialog zur Erstellung einer Regel aus dem Regelsatz 'Oracle Tablespaces'."]
...
image::rule_ora_properties.png[alt="Dialog zur Festlegung der Eigenschaften für die neue Regel."]
...
image::rule_ora_value.png[alt="Dialog zur Festlegung der Werte für die neue Regel."]
...
image::rule_ora_condition.png[alt="Dialog zur Festlegung der Bedingungen für die neue Regel."]
...
* Die eingegebenen Texte werden immer gegen den Anfang gematcht. Die Eingabe von `DW20` greift also auch auf einen fiktiven Tablespace `DW20A`. Wenn Sie das verhindern möchten, hängen Sie das Zeichen `$` an das Ende an, also `DW20$`, denn es handelt sich hier um sogenannte xref:regexes#[reguläre Ausdrücke].

*Hinweis:* Die detaillierte Beschreibung aller anderen Parameter und eine ausführliche Erklärung zum wichtigen Konzept der Regeln finden Sie im xref:wato_rules#[Artikel über Regeln]. Mehr zu den [.guihint]#Service labels#, dem letzten Parameter im obigen Bild, erfahren Sie übrigens im xref:labels#[Artikel über Labels].
...
image::wato_tag_groups.png[alt="Liste der Host-Merkmalsgruppen."]
...
image::new_taggroup_basic.png[alt="Dialog zur Festlegung der Grundeinstellungen für die neue Host-Merkmalsgruppe."]
...
image::new_taggroup_choices.png[alt="Dialog zur Festlegung der Merkmale für die neue Host-Merkmalsgruppe."]
...
* An dieser Stelle können Sie die Hilfsmerkmale (_auxiliary tags_) erstmal ignorieren. Sie erhalten alle Informationen zu Hilfsmerkmalen im Speziellen und zu Host-Merkmalen im Allgemeinen im xref:wato_rules#[Artikel über Regeln].
...
image::host_custom_attributes.png[alt="Dialog mit Eigenschaften eines Hosts mit der neuen Host-Merkmalsgruppe."]
...
Überprüfen Sie die Schwellwerte für die Überwachung von Dateisystemen und passen Sie diese gegebenenfalls an. Die Standardwerte hatten wir bereits   weiter oben bei der xref:intro_finetune#find_rules[Suche nach Regeln] kurz gezeigt.
...
* Die Parameter erlauben Schwellwerte, die von der Größe des Dateisystems abhängen. Wählen Sie dazu [.guihint]#Levels for filesystem > Levels for filesystem used space > Dynamic levels#. Mit dem Knopf [.guihint]#Add new element# definieren Sie jetzt pro Plattengröße eigene Schwellwerte.
...
{cre-only}
In der {RE} definieren Sie zunächst eine Zeitperiode (_time period_), welche die Zeiten des Reboots abdeckt. Wie das geht, erfahren Sie im xref:timeperiods#[Artikel über Zeitperioden]. Danach legen Sie jeweils eine Regel in den Regelsätzen [.guihint]#Notification period for hosts# und [.guihint]#Notification period for services# für die betroffenen Hosts an und wählen dort die zuvor definierte Zeitperiode aus. Die zweite Regel für die Services ist nötig, damit auch Services, die in dieser Zeit auf {CRIT} gehen, keine Benachrichtigungen auslösen. Falls nun während dieser Zeiten Probleme auftreten (und rechtzeitig wieder verschwinden), werden keine Benachrichtigungen ausgelöst.
...
mit {CMK}. In aller Regel ist aber das Ausschalten eines Druckers vor Feierabend kein Problem. Dumm ist nur, wenn {CMK} hier benachrichtigt, weil der entsprechende Host {DOWN} ist.
...
image::host_check_command.png[alt="Dialog zur Festlegung des 'Host Check Command' in der gleichnamigen Regel."]
...
image::rule_status_of_cmk_services.png[alt="Dialog zur Festlegung des Zustands von '{CMK} Services' in einer Regel."]
...
image::port_discovery.png[alt="Dialog zur Festlegung der Überwachung von Switch Ports in einer Regel."]
...
image::cpu_util_average.png[alt="Dialog zur Festlegung von Mittelwerten für die CPU-Auslastung in einer Regel."]
...
image::discovery_service.png[Service-Liste mit dem Service 'Check_MK Discovery'.]
...
*Tipp:* Über [.guihint]#Monitor > System > Unmonitored services# können Sie sich eine Ansicht aufrufen, die Ihnen die neuen und weggefallenen Services anzeigt.
