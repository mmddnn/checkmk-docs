...
The rules are organized in _rule sets_. For each type of parameter,
...
that sets the thresholds for all services that monitor file systems. To implement
...
xref:intro_gui#search_setup[search field in the setup menu] or by opening the rule
...
click on the icon:icon_menu[alt="Icon for opening the action menu."] menu option and select the [.guihint]#Parameters for this service# entry:

image::service_rule_icon.png[alt="List entry of a service with open action menu."]

You will receive a page from which you can access all of the rule sets for this service:

image::parameters_of_this_service.png[alt="List of all rule sets for a service."]
...
image::ruleset_search_tablespace.png[alt="Result of the search for 'tablespace' in rules."]
...
image::ruleset_oracle_tablespaces.png[alt="Dialogue for creating a rule from the 'Oracle Tablespaces' rule set."]
...
image::rule_ora_properties.png[alt="Dialogue for setting the properties for the new rule."]
...
image::rule_ora_value.png[alt="Dialogue for setting the values for the new rule."]
...
image::rule_ora_condition.png[alt="Dialogue for setting the conditions for the new rule."]
...
* The entered texts are always matched against the beginning. The input of `DW20` therefore also accesses a fictitious tablespace `DW20A`. If you want to prevent this, append the `$` character to the end, i.e. `DW20$`, because these are so-called xref:regexes#[regular expressions].

*Note:* A detailed description of all of the other parameters and a detailed
explanation of the important concept of rules can be found in the xref:wato_rules#[article on rules].
By the way, you can find out more about the [.guihint]#service labels#,
the last parameter in the image above, in the xref:labels#[article on labels].
...
image::ruleset_ora_one_rule.png[alt="List of rules with the new rule created."]
...
image::wato_tag_groups.png[alt="List of host tag groups."]
...
image::new_taggroup_basic.png[alt="Dialogue for defining the 'Basic settings' for the new host tag group."]
...
image::new_taggroup_choices.png[alt="Dialogue for setting the tags for the new host tag group."]
...
* At this point, you can ignore the _auxiliary tags_ for now. You can get all the information on auxiliary tags in particular and on host tags in general in the xref:wato_rules#[article on rules].
...
image::host_custom_attributes.png[alt="Dialogue with properties of a host with the new host tag group."]
...
xref:intro_finetune#find_rules[search for rules].
...
* The parameters allow thresholds that depend on the size of the file system. To do this, select [.guihint]#Levels for filesystem > Levels for filesystem used space > Dynamic levels#. With the [.guihint]#Add new element# button you can now define your own threshold values per disk size.
...
{cre-only}
In the {RE} you first define a _time period_ that covers the times of
the reboot. You can find out how to do this in the xref:timeperiods#[article on time periods].
...
{cee-only}
In the {EE} there are regular scheduled downtimes for this purpose that you
can set for any affected hosts.

*Tip:* An alternative to creating downtimes for hosts, which we have
already described in the xref:intro_monitor#downtimes[chapter on scheduled downtimes],
...
sense -- some users even organize the reordering of toner using {CMK}.
...
image::host_check_command.png[alt="Dialogue for setting the 'Host Check Command' in the rule of the same name."]
...
image::rule_status_of_cmk_services.png[alt="Dialogue for setting the state of '{CMK} Services' in a rule."]
...
image::port_discovery.png[alt="Dialogue for defining the monitoring of switch ports in a rule."]
...
image::cpu_util_average.png[alt="Dialogue for setting mean values for CPU utilization in a rule."]
...
image::discovery_service.png[alt="Service list with the 'Check_MK Discovery' service."]
...
*Tip:* With [.guihint]#Monitor > System > Unmonitored services# you can call up a view that shows you any new or dropped services.
